/*
 * freertos_twi_lis3dh.c
 *
 * Created: 22/10/2557 11:30:22
 *  Author: pan
 */ 
#include <adl_global.h>
#include "../freertos_twi_lsm6ds3.h"

static freertos_twi_if freertos_twi_lsm6ds3_hBus;
...
)
{
	...
	write_parameters.chip = BOARD_LSM6DS3_ADDRESS;
...
freertos_twi_if freertos_twi_lsm6ds3_init(Twi *twi_base) {
	...
	freertos_twi_lsm6ds3_hBus = freertos_twi_master_init(twi_base,&blocking_driver_options);
	/* Check the port was initialized successfully. */
	configASSERT(freertos_twi_lsm6ds3_hBus);
	/* Configure the TWI bus parameters.  Do this after calling
	freertos_twi_master_init(). */
	twi_set_speed(twi_base, FREERTOS_TWI_LSM6DS3_CLOCK_HZ, sysclk_get_cpu_hz());
	return freertos_twi_lsm6ds3_hBus;
}
