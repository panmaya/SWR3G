...
#include "../freertos_twi_lis3dh.h"

static freertos_twi_if freertos_twi_lis3dh_hBus;
...
)
{
	...
	write_parameters.chip = BOARD_LIS3DHC_ADDRESS;
...
freertos_twi_if freertos_twi_ls3dh_init(Twi *twi_base) {
	...
	freertos_twi_lis3dh_hBus = freertos_twi_master_init(twi_base,&blocking_driver_options);
	/* Check the port was initialized successfully. */
	configASSERT(freertos_twi_lis3dh_hBus);
	/* Configure the TWI bus parameters.  Do this after calling
	freertos_twi_master_init(). */
	twi_set_speed(twi_base, FREERTOS_TWI_LIS3DH_CLOCK_HZ, sysclk_get_cpu_hz());
	return freertos_twi_lis3dh_hBus;
}
